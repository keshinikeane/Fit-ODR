...
def model(P, x): # Define the model to be used to fit the data. The first input is the array of parameters (to be fit), and the second is the independent variable, x.
    return P[0]+P[1]*np.exp(P[2]*x) # Provide the functional form of the model, including the parameters, as shown. P[0] + P[1]/x**2. + P[2]/x**3.
...
# Specify the formatted model and data (above), in addition to the initial guesses for the parameters.  The length of the parameter array, beta0, must equal the number of paramters in the model.

# Perform Orthogonal Distance Regression
myodr = odrpack.ODR(mydata, mymodel, beta0=[0., 0., 0.]) 
